...
// Include the file where generateVerificationCode() and sendVerificationCode() are defined
// require_once 'path_to_your_functions.php'; 
...
if (isset($_POST['login'])) {
    ...
    // Admin login check
    $stmt = $conn->prepare("SELECT admin_id, name, password FROM indexlogin WHERE admin_id = ?");
    ...
    $stmt = $conn->prepare("SELECT user_id, name, password, role, email FROM users WHERE BINARY name = ?");
    $stmt->bind_param("s", $name);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
        $stmt->bind_result($employee_id, $employee_user, $db_password, $role, $email);
        $stmt->fetch();
        ...
        if (password_verify($password, $db_password)) {
            ...
            if (!empty($email)) {
                ...
                if (sendVerificationCode($email, $employee_user, $verificationCode)) {
                    // Redirect to verification page
                    header("Location: verify_code.php");
                    exit;
                } else {
                    // Failed to send email
                    echo "<script>alert('Failed to send verification code. Please contact administrator.');</script>";
                }
            } else {
                // No email on file - admin needs to add email
                echo "<script>alert('No email address found for your account. Please contact administrator.');</script>";
                session_unset();
            }
        } else {
...
<body id="page-top">
    ...
    <!-- Masthead-->
    <header class="masthead" style="background-image: url('pics/admin_main-bg2.jpg');">
        <div class="container">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
              ...
              <form action="" method="POST" style="max-width: 400px; margin: auto;">
                <img src="admin/pics/rcgiph_logo.png" alt="logo" style="width: 80px; margin-bottom: 20px;" />
